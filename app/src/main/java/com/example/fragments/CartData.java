...
public class CartData {
    private static String[] namaCart = {
            "Ahmad Dahlan",
            "Ahmad Yani",
            "Sutomo",
            "Gatot Soebroto",
    ...
    private static String[] detailCart = {
    ...
    public static ArrayList<CartModel> getCartList(){
        CartModel CartModel = null;
        ArrayList<CartModel> list = new ArrayList<>();

        for (int i = 0; i < namaCart.length; i++){
            CartModel = new CartModel();
            CartModel.setNama(namaCart[i]);
            CartModel.setDes(detailCart[i]);
            CartModel.setThumb(imageCart[i]);
